...
public class DungeonStartMercury extends RoomEntranceMercury
{
    ...
    public DungeonStartMercury()
    {
    }

    public DungeonStartMercury(World world, DungeonConfiguration configuration, Random rand, int blockPosX, int blockPosZ)
    {
    ...
    public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand)
    {
        ...
        while (!validAttempt && attempts < maxAttempts)
        {
            ...
            listIn.clear();
            listIn.add(this);
            PieceMercury next = getNextPiece(this, rand);
            while (next != null)
            {
                listIn.add(next);
            ...
            if (attachedComponents.size() >= 3 && attachedComponents.get(attachedComponents.size() - 1) instanceof RoomTreasureMercury &&
                    attachedComponents.get(attachedComponents.size() - 3) instanceof RoomBossMercury)
            {
                validAttempt = true;
            }
            attempts++;
