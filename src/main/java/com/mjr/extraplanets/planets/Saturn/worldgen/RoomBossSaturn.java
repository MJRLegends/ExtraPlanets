...
public class RoomBossSaturn extends RoomBoss {
	...
	public boolean addComponentParts(World world, Random rand, StructureBoundingBox chunkBox) {
		...
		if (chunkBox.isVecInside(blockpos)) {
			world.setBlockState(blockpos, ExtraPlanets_Blocks.SATURN_SPAWNER.getDefaultState(), 2);
			if (Config.USE_DEFAULT_BOSSES) {
				TileEntityDungeonSpawnerSaturnDefault spawner = (TileEntityDungeonSpawnerSaturnDefault) world.getTileEntity(blockpos);
				if (spawner != null) {
				...
				TileEntityDungeonSpawnerSaturn spawner = (TileEntityDungeonSpawnerSaturn) world.getTileEntity(blockpos);
				if (spawner != null) {
					spawner.setRoom(new Vector3(this.boundingBox.minX + 1, this.boundingBox.minY + 1, this.boundingBox.minZ + 1), new Vector3(this.sizeX - 1, this.sizeY - 1, this.sizeZ - 1));
				}
			}
