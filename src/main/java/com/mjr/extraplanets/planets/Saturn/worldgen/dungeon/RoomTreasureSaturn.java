...
public class RoomTreasureSaturn extends DungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public RoomTreasureSaturn(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	public void generate(Block[] chunk, byte[] meta, int cx, int cz)
	{
		...
		if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, ExtraPlanets_Blocks.treasureChestTier6, 0))
		{
	...
	{
		return new RoomTreasureSaturn(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		if (!this.chests.isEmpty())
		{
			...
			{
				this.worldObj.setBlock(coords.posX, coords.posY, coords.posZ, ExtraPlanets_Blocks.treasureChestTier6, 0, 3);
				this.worldObj.setTileEntity(coords.posX, coords.posY, coords.posZ, new TileEntityT6TreasureChest(6));
