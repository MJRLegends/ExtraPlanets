...
public class BiomeDecoratorKepler22b extends BiomeDecorator {
	...
	protected void genDecorations(BiomeGenBase biomeGenBaseIn) {
		...
		for (int i = 0; i < blueTreesPerChunk; i++) {
			BlockPos blockpos = this.currentWorld.getTopSolidOrLiquidBlock(new BlockPos(this.field_180294_c.getX() + 8, 0, this.field_180294_c.getZ() + 8));
		...
		for (int i = 0; i < 1; i++) {
			if (this.randomGenerator.nextInt(200) == 1) {
				BlockPos blockpos = this.currentWorld.getTopSolidOrLiquidBlock(new BlockPos(this.field_180294_c.getX() + (this.randomGenerator.nextInt(16) + 8), 0, this.field_180294_c.getZ() + (this.randomGenerator.nextInt(16) + 8)));
				blockpos.down(2);
