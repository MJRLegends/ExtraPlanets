			...
			for (int j = 0; j <= this.sizeY; j++) {
				for (int k = 0; k <= this.sizeZ; k++) {
					if (i == 0 || i == this.sizeX || j == 0 || j == this.sizeY || k == 0 || k == this.sizeZ) {
						...
						this.setBlockState(world, ExtraPlanets_Blocks.TREASURE_CHEST_TIER_10.getDefaultState().withProperty(BlockTier1TreasureChest.FACING, this.getDirection().getOpposite()), i, j, k, boundingBox);
					} else {
						this.setBlockState(world, Blocks.air.getDefaultState(), i, j, k, boundingBox);
					}
				}
