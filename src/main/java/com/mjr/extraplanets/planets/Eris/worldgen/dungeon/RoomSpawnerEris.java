...
public class RoomSpawnerEris extends DungeonRoom {
	int sizeX;
	int sizeY;
	int sizeZ;
	Random rand;
	...
	public RoomSpawnerEris(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir) {
	...
	protected DungeonRoom makeRoom(MapGenDungeon dungeon, int x, int y, int z, ForgeDirection dir) {
		return new RoomSpawnerEris(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand) {
		for (final ChunkCoordinates spawnerCoords : this.spawners) {
			if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Blocks.mob_spawner) {
				...
				if (spawner != null) {
					spawner.func_145881_a().setEntityName(RoomSpawnerEris.getMob(rand));
				}
			}
		}
	}
