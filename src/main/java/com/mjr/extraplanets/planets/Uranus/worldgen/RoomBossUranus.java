...
public class RoomBossUranus extends RoomBoss {
	...
	public RoomBossUranus(DungeonConfiguration configuration, Random rand, int blockPosX, int blockPosZ, int sizeX, int sizeY, int sizeZ, EnumFacing entranceDir) {
		super(configuration, rand, blockPosX, blockPosZ, sizeX, sizeY, sizeZ, entranceDir);
	}

	public RoomBossUranus(DungeonConfiguration configuration, Random rand, int blockPosX, int blockPosZ, EnumFacing entranceDir) {
	...
	public boolean addComponentParts(World world, Random rand, StructureBoundingBox chunkBox) {
		...
		if (chunkBox.isVecInside(blockpos)) {
			world.setBlockState(blockpos, ExtraPlanets_Blocks.URANUS_SPAWNER.getDefaultState(), 2);
			if (Config.USE_DEFAULT_BOSSES) {
				TileEntityDungeonSpawnerUranusDefault spawner = (TileEntityDungeonSpawnerUranusDefault) world.getTileEntity(blockpos);
				if (spawner != null) {
				...
				TileEntityDungeonSpawnerUranus spawner = (TileEntityDungeonSpawnerUranus) world.getTileEntity(blockpos);
				if (spawner != null) {
					spawner.setRoom(new Vector3(this.boundingBox.minX + 1, this.boundingBox.minY + 1, this.boundingBox.minZ + 1), new Vector3(this.sizeX - 1, this.sizeY - 1, this.sizeZ - 1));
				}
			}
