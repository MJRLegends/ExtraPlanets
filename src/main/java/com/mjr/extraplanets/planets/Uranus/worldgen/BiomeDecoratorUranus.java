	...
	protected void decorate() {
		...
		for (int i = 0; i < this.LakesPerChunk; i++) {
			if (this.rand.nextInt(10) == 0) {
				WorldGenUtilities.generateLake(this.currentWorld, this.rand, new BlockPos(this.chunkX, 0, this.chunkZ), ExtraPlanets_Fluids.FROZEN_WATER, ExtraPlanets_Blocks.URANUS_BLOCKS);
			}
		}
		...
		if (Config.GENERATE_URANUS_IGLOOS) {
			if (this.rand.nextInt(300) == 1) {
				WorldGenUtilities.generateStructure(new WorldGenIgloo(), this.currentWorld, this.rand, new BlockPos(this.chunkX, 0, this.chunkZ));
			}
		}
