...
import com.mjr.extraplanets.planets.Uranus.worldgen.biomes.GenLayerUranus;

public class BiomeProviderUranus extends BiomeProvider {
	private GenLayer unzoomedBiomes;
	private GenLayer zoomedBiomes;
	private BiomeCache biomeCache;
	private List<Biome> biomesToSpawnIn;

	protected BiomeProviderUranus() {
		biomeCache = new BiomeCache(this);
		biomesToSpawnIn = new ArrayList<>();
	}

	public BiomeProviderUranus(long seed, WorldType type) {
		this();
		GenLayer[] genLayers = GenLayerUranus.createWorld(seed);
		this.unzoomedBiomes = genLayers[0];
		this.zoomedBiomes = genLayers[1];
	}

	public BiomeProviderUranus(World world) {
	...
	public Biome getBiome(BlockPos pos, Biome defaultBiome) {
		return this.biomeCache.getBiome(pos.getX(), pos.getZ(), UranusBiomes.uranus);
	}
	...
	public Biome[] getBiomes(@Nullable Biome[] listToReuse, int x, int z, int width, int length, boolean cacheFlag) {
		...
		for (int i = 0; i < width * length; ++i) {
			if (zoomed[i] >= 0) {
				...
				listToReuse[i] = UranusBiomes.uranus;
			}
		}
