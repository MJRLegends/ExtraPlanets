...
import com.mjr.extraplanets.planets.Neptune.worldgen.layer.GenLayerNeptune;

public class WorldChunkManagerNeptuneNew extends WorldChunkManagerSpace {
	...
	public WorldChunkManagerNeptuneNew(long seed) {
		this();
		GenLayer[] agenlayer;
		agenlayer = GenLayerNeptune.makeTheWorld(seed);
		this.unzoomedBiomes = agenlayer[0];
		this.zoomedBiomes = agenlayer[1];
	}

	public WorldChunkManagerNeptuneNew(World world) {
		this(world.getSeed());
	}
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int z, int length, int width) {
		...
		for (int i = 0; i < length * width; i++) {
			if (arrayOfInts[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = BiomeGenBaseNeptune.neptune;
			}
		}
