...
public class RoomBossNeptune extends DungeonRoom {
	public int sizeX;
	public int sizeY;
	public int sizeZ;
	Random rand;
	...
	public RoomBossNeptune(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir) {
	...
	protected DungeonRoom makeRoom(MapGenDungeon dungeon, int x, int y, int z, ForgeDirection dir) {
		return new RoomBossNeptune(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand) {
		...
		if (Config.useDefaultBosses) {
			if (tile == null || !(tile instanceof TileEntityDungeonSpawnerNeptuneDefault)) {
				TileEntityDungeonSpawnerNeptuneDefault spawner = new TileEntityDungeonSpawnerNeptuneDefault();
			...
			if (tile == null || !(tile instanceof TileEntityDungeonSpawnerNeptune)) {
				TileEntityDungeonSpawnerNeptune spawner = new TileEntityDungeonSpawnerNeptune();
				spawner.setRoom(new Vector3(this.posX, this.posY, this.posZ), new Vector3(this.sizeX, this.sizeY, this.sizeZ));
				this.worldObj.setTileEntity(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, spawner);
			} else if (tile instanceof TileEntityDungeonSpawner) {
				((TileEntityDungeonSpawner) tile).setRoom(new Vector3(this.posX, this.posY, this.posZ), new Vector3(this.sizeX, this.sizeY, this.sizeZ));
