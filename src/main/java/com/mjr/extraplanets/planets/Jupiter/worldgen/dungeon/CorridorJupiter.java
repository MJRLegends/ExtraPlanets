...
public class CorridorJupiter extends SizedPieceJupiter
{
    public CorridorJupiter()
    {
    }
	...
	private <T extends SizedPieceJupiter> T getRoom(Class<?> clazz, DungeonStartJupiter startPiece, Random rand)
    {
        try
        {
    ...
    public PieceJupiter getNextPiece(DungeonStartJupiter startPiece, Random rand)
    {
        ...
        if (pieceCount > 10 && startPiece.attachedComponents.get(pieceCount - 2) instanceof RoomBossJupiter)
        {
            try
            {
        ...
        else
        {
            ...
            boolean bossRoom = pieceCount > 25 || (pieceCount > 20 && rand.nextInt(bossRoomChance) == 0);
            if (bossRoom)
            {
				try {
            ...
            else
            {
                ...
                case 1:
                case 2:
                    return new RoomChestJupiter(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
                case 3:
                case 4:
                default:
                    return new RoomSpawnerJupiter(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
                    //return new RoomEmptyJupiter(this.configuration, rand, blockX, blockZ, sizeX, sizeY, sizeZ, this.getDirection().getOpposite());
                }
            }
