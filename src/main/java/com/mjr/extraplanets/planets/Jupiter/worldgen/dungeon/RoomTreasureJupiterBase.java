...
public class RoomTreasureJupiterBase extends SizedPieceJupiter
{
    ...
    public RoomTreasureJupiterBase(DungeonConfiguration configuration, Random rand, int blockPosX, int blockPosZ, EnumFacing entranceDir)
    {
    ...
    public PieceJupiter getNextPiece(DungeonStartJupiter startPiece, Random rand)
    {
        if (startPiece.attachedComponents.size() > 2)
        {
            ...
            if (component instanceof RoomBossJupiter)
            {
                ...
                ((RoomBossJupiter) component).setChestPos(new BlockPos(blockpos));
            }
        }
        return null;
    }
