...
public class DungeonStartJupiter extends EntranceCraterJupiter
{
    ...
    public void buildComponent(StructureComponent componentIn, List<StructureComponent> listIn, Random rand)
    {
        ...
        while (!validAttempt && attempts < maxAttempts)
        {
            ...
            if (attachedComponents.size() >= 3 && attachedComponents.get(attachedComponents.size() - 1) instanceof RoomTreasureJupiter &&
                    attachedComponents.get(attachedComponents.size() - 3) instanceof RoomBossJupiter)
            {
                validAttempt = true;
            }
            attempts++;
