...
public class RoomEntranceJupiter extends SizedPieceJupiter {
    ...
    public RoomEntranceJupiter(World world, DungeonConfiguration configuration, Random rand, int blockPosX, int blockPosZ)
    {
        ...
        for (int i = startX; i <= endX; i++)
        {
            for (int j = 0; j <= this.sizeY; j++)
            {
                for (int k = startZ; k <= endZ; k++)
                {
                    ...
                    {
                        this.setBlockState(worldIn, j == 0 || j == this.sizeY ? this.configuration.getBrickBlock() : this.configuration.getBrickBlock(), i, j, k, boundingBox);
                    }
                    else
                    {
