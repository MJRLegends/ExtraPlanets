...
public class RoomSpawnerJupiter extends DungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public RoomSpawnerJupiter(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
	...
	{
		return new RoomSpawnerJupiter(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (final ChunkCoordinates spawnerCoords : this.spawners)
		{
			if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Blocks.mob_spawner)
			{
				...
				if (spawner != null)
				{
					spawner.func_145881_a().setEntityName(RoomSpawnerJupiter.getMob(rand));
				}
			}
		}
	}
