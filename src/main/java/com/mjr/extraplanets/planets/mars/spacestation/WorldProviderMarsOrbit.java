...
public class WorldProviderMarsOrbit extends WorldProviderSpace implements IOrbitDimension, ISolarLevel, IExitHeight
{
	public int spaceStationDimensionID;

	private static final float GFORCE = 9.81F / 400F; //gravity in metres per tick squared

	private OrbitSpinSaveDataMars savefile;
	...
	public void updateWeather()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			{
				this.savefile = OrbitSpinSaveDataMars.initWorldData(this.worldObj);
	...
	public void updateSpinSpeed()
	{
		if (this.momentOfInertia > 0F)
		{
			...
			else
			{
				...
				this.angularVelocityTarget = MathHelper.sqrt_float(WorldProviderMarsOrbit.GFORCE / maxR) / 2;
