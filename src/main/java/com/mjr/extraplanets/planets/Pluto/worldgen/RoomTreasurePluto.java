				...
				for (int k = 0; k <= this.sizeZ; k++) {
					if (i == 0 || i == this.sizeX || j == 0 || j == this.sizeY || k == 0 || k == this.sizeZ) {
						...
						if (boundingBox.isVecInside(blockpos)) {
							world.setBlockState(blockpos, ExtraPlanets_Blocks.TREASURE_CHEST_TIER_9.getDefaultState().withProperty(BlockTier1TreasureChest.FACING, this.getDirection().getOpposite()), 2);
