...
public class MapGenCaveCallisto extends MapGenBaseMeta {
	private Block grassblock = ExtraPlanets_Blocks.callistoBlocks;
	private Block dirtblock = ExtraPlanets_Blocks.callistoBlocks;
	private Block stoneblock = ExtraPlanets_Blocks.callistoBlocks;
	...
	protected void generateCaveNode(long par1, int par3, int par4, Block[] blockIdArray, byte[] metaArray, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
		...
		for (final boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
			...
			if (!flag && par15 == k1 && par12 > 1.0F && par16 > 0) {
				this.generateCaveNode(random.nextLong(), par3, par4, blockIdArray, metaArray, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 - Constants.floatPI / 2F, par14 / 3.0F, par15, par16, 1.0D);
				this.generateCaveNode(random.nextLong(), par3, par4, blockIdArray, metaArray, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 + Constants.floatPI / 2F, par14 / 3.0F, par15, par16, 1.0D);
				return;
			}
