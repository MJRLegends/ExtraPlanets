...
public class WorldChunkManagerCallisto extends WorldChunkManager {
	private GenLayer unzoomedBiomes;
	private GenLayer zoomedBiomes;
	private BiomeCache biomeCache;
	private List<BiomeGenBase> biomesToSpawnIn;

	protected WorldChunkManagerCallisto() {
		biomeCache = new BiomeCache(this);
		biomesToSpawnIn = new ArrayList<>();
	}

	public WorldChunkManagerCallisto(long seed, WorldType type) {
		this();
		GenLayer[] genLayers = GenLayerCallisto.createWorld(seed);
		this.unzoomedBiomes = genLayers[0];
		this.zoomedBiomes = genLayers[1];
	}

	public WorldChunkManagerCallisto(World world) {
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] biomes, int x, int z, int length, int width) {
		...
		for (int i = 0; i < length * width; ++i) {
			if (intArray[i] >= 0) {
				...
				biomes[i] = CallistoBiomes.callisto;
			}
		}
	...
	public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] listToReuse, int x, int z, int length, int width, boolean cacheFlag) {
		...
		for (int i = 0; i < width * length; ++i) {
			if (zoomed[i] >= 0) {
				...
				listToReuse[i] = CallistoBiomes.callisto;
			}
		}
