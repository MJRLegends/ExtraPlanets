		...
		if (!ridingAutoRocket && player != null) {
			...
			if (stats.getTeleportCooldown() <= 0) {
				...
				if (!newWorld.isRemote) {
					...
					LanderUtil.makeSmallLandingSpot(newWorld, (int)lander.posX, (int)lander.posZ, ExtraPlanets_Blocks.IO_BLOCKS.getDefaultState().withProperty(BlockBasicIo.BASIC_TYPE, BlockBasicIo.EnumBlockBasic.STONE), false);
