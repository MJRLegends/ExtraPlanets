...
public class WorldChunkManagerOberon extends WorldChunkManager {
	private GenLayer unzoomedBiomes;
	private GenLayer zoomedBiomes;
	private BiomeCache biomeCache;
	private List<BiomeGenBase> biomesToSpawnIn;

	protected WorldChunkManagerOberon() {
		biomeCache = new BiomeCache(this);
		biomesToSpawnIn = new ArrayList<>();
	}

	public WorldChunkManagerOberon(long seed, WorldType type) {
		this();
		GenLayer[] genLayers = GenLayerOberon.createWorld(seed);
		this.unzoomedBiomes = genLayers[0];
		this.zoomedBiomes = genLayers[1];
	}

	public WorldChunkManagerOberon(World world) {
	...
	public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] listToReuse, int x, int z, int length, int width, boolean cacheFlag) {
		...
		for (int i = 0; i < width * length; ++i) {
			if (zoomed[i] >= 0) {
				...
				listToReuse[i] = OberonBiomes.oberon;
			}
		}
