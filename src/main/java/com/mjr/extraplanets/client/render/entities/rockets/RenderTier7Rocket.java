...
public class RenderTier7Rocket extends Render<EntityTier7Rocket> {
	private ItemModelRocketT7 rocketModel;

	public RenderTier7Rocket(RenderManager manager) {
		super(manager);
		this.shadowSize = 2F;
	}
	...
	private void updateModel() {
		if (rocketModel == null) {
			...
			rocketModel = (ItemModelRocketT7) FMLClientHandler.instance().getClient().getRenderItem().getItemModelMesher().getModelManager().getModel(modelResourceLocation);
		}
	}

	@Override
	protected ResourceLocation getEntityTexture(EntityTier7Rocket par1Entity) {
		return new ResourceLocation("missing");
	}

	@Override
	public void doRender(EntityTier7Rocket entity, double par2, double par4, double par6, float par8, float par9) {
		...
		// RenderHelper.di sableStandardItemLighting();
