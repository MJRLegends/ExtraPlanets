...
public class ItemRendererTier4Rocket implements IItemRenderer
{
	...
	protected ResourceLocation texture = new ResourceLocation(Constants.ASSET_PREFIX, "textures/model/blank_rocket.png");
	protected ResourceLocation texture2 = new ResourceLocation(Constants.ASSET_PREFIX, "textures/model/blank_rocket_dark_grey.png");
	...
	protected void renderSpaceship(ItemRenderType type, RenderBlocks render, ItemStack item, float translateX, float translateY, float translateZ)
	{
		...
		if (type == ItemRenderType.INVENTORY)
		{
			...
			if (EnumRocketType.values()[index].getInventorySpace() > 3)
			{
				...
				FMLClientHandler.instance().getClient().renderEngine.bindTexture(ItemRendererTier4Rocket.chestTexture);
	...
	public void transform(ItemStack itemstack, ItemRenderType type)
	{
		...
		if (type == ItemRenderType.EQUIPPED)
		{
			...
			GL11.glTranslatef(-1.0F, -7.2F, 1F);
		...
		if (type == ItemRenderType.INVENTORY || type == ItemRenderType.ENTITY)
		{
			if (type == ItemRenderType.INVENTORY)
			{
				...
				GL11.glTranslatef(0.5F, 2.6F, -0.4F);
			}
			else
			{
