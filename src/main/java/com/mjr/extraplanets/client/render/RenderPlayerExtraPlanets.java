package com.mjr.extraplanets.client.render;
...
public class RenderPlayerExtraPlanets extends RenderPlayerGC {
	...
	public RenderPlayerExtraPlanets() {
		super();
		...
		if (GalacticraftCore.isPlanetsLoaded) {
			RenderPlayerExtraPlanets.thermalPaddingTexture0 = new ResourceLocation("galacticraftasteroids", "textures/misc/thermalPadding_0.png");
			RenderPlayerExtraPlanets.thermalPaddingTexture1 = new ResourceLocation("galacticraftasteroids", "textures/misc/thermalPadding_1.png");
		}
	}
	...
	public static void renderModelS(RendererLivingEntity inst, EntityLivingBase par1EntityLivingBase, float par2, float par3, float par4, float par5, float par6, float par7) {
		if (inst instanceof RenderPlayer) {
			...
			if (RenderPlayerExtraPlanets.thermalPaddingTexture0 != null && !isSmartRenderLoaded) {
				...
				if (gearData != null && !RenderPlayerGC.flagThermalOverride) {
					...
					for (int i = 0; i < 4; ++i) {
						...
						if (padding >= 0 && !par1EntityLivingBase.isInvisible()) {
							...
							Minecraft.getMinecraft().renderEngine.bindTexture(RenderPlayerExtraPlanets.thermalPaddingTexture1);
							...
							/*
							 * modelBiped.onGround =
							 * thisInst.mainModel.onGround; modelBiped.isRiding
							 * = thisInst.mainModel.isRiding; modelBiped.isChild
							 * = thisInst.mainModel.isChild; if
							 * (thisInst.mainModel instanceof ModelBiped) {
							 * modelBiped.heldItemLeft = ((ModelBiped)
							 * thisInst.mainModel).heldItemLeft;
							 * modelBiped.heldItemRight = ((ModelBiped)
							 * thisInst.mainModel).heldItemRight;
							 * modelBiped.isSneak = ((ModelBiped)
							 * thisInst.mainModel).isSneak; modelBiped.aimedBow
							 * = ((ModelBiped) thisInst.mainModel).aimedBow; }
							 */
							...
							Minecraft.getMinecraft().renderEngine.bindTexture(RenderPlayerExtraPlanets.thermalPaddingTexture0);
