			...
			if (stack.getItem() instanceof UniversalBucket) {
				if (FluidUtil.getFluidContained(stack) != null && FluidUtil.getFluidContained(stack).getFluid() != null) {
					...
					if (fluid.equals(ExtraPlanets_Fluids.FROZEN_WATER_FLUID) || fluid.equals(ExtraPlanets_Fluids.GLOWSTONE_FLUID) || fluid.equals(ExtraPlanets_Fluids.INFECTED_WATER_FLUID) || fluid.equals(ExtraPlanets_Fluids.LIQUID_HYDROCARBON_FLUID)
						...
						event.getToolTip().add(EnumColor.AQUA + GCCoreUtil.translate("gui.bucket.message.finding.2." + fluid.getUnlocalizedName().substring(6)));
				...
				event.getToolTip().add(EnumColor.AQUA + GCCoreUtil.translate("gui.message.decontamination.unit"));
			}
		}
	}
	...
	public void onGuiOpenEvent(GuiOpenEvent event) {
		if (((event.getGui() instanceof GuiCelestialSelection))) {
			if (GameSettings.isKeyDown(micdoodle8.mods.galacticraft.core.tick.KeyHandlerClient.galaxyMap) && Config.USE_CUSTOM_CELESTAIAL_SELECTION) {
				event.setGui(new CustomCelestialSelection(true, null, PermissionAPI.hasPermission(Minecraft.getMinecraft().player, Constants.PERMISSION_CREATE_STATION)));
			} else {
				event.setGui(new CustomCelestialSelection(false, null, PermissionAPI.hasPermission(Minecraft.getMinecraft().player, Constants.PERMISSION_CREATE_STATION)));
			}
		}
	}
