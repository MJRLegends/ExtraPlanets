...
public class ItemModelRocketT5 extends ModelTransformWrapper
{
    public ItemModelRocketT5(IBakedModel modelToWrap)
    {
        super(modelToWrap);
    }
    ...
    protected Matrix4f getTransformForPerspective(TransformType cameraTransformType)
    {
        if (cameraTransformType == TransformType.GUI)
        {
            ...
            ret.mul(mul);
            mul.setIdentity();
            mul.setTranslation(new Vector3f(-1.0F, -0.15F, -0.15F));
            ret.mul(mul);
            mul.setIdentity();
        ...
        if (cameraTransformType == TransformType.FIRST_PERSON)
        {
            ...
            Vector3f trans = new Vector3f(0.5F, -1.9F, -0.8F);
            Matrix4f ret = new Matrix4f();
            ret.setIdentity();
            Matrix4f mul = new Matrix4f();
            mul.setIdentity();
        ...
        if (cameraTransformType == TransformType.THIRD_PERSON)
        {
            ...
            Vector3f trans = new Vector3f(0.0F, -1.4F, -0.4F);
            Matrix4f ret = new Matrix4f();
            ret.setIdentity();
            Matrix4f mul = new Matrix4f();
            mul.setIdentity();
