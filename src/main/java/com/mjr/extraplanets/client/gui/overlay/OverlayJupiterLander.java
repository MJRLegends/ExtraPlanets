...
public class OverlayJupiterLander extends Overlay {
	...
	public static void renderLanderOverlay() {
		...
		if (OverlayJupiterLander.minecraft.player.getRidingEntity().motionY < -2.0) {
			OverlayJupiterLander.minecraft.fontRenderer.drawString(TranslateUtilities.translate("gui.warning"), width / 4 - OverlayJupiterLander.minecraft.fontRenderer.getStringWidth(TranslateUtilities.translate("gui.warning")) / 2, height / 8 - 20,
			...
			OverlayJupiterLander.minecraft.fontRenderer.drawString(press1 + GameSettings.getKeyDisplayString(KeyHandlerClient.spaceKey.getKeyCode()) + press2,
					width / 4 - OverlayJupiterLander.minecraft.fontRenderer.getStringWidth(press1 + GameSettings.getKeyDisplayString(KeyHandlerClient.spaceKey.getKeyCode()) + press2) / 2, height / 8,
		...
		if (OverlayJupiterLander.minecraft.player.getRidingEntity().motionY != 0.0D) {
			String string = TranslateUtilities.translate("gui.lander.velocity") + ": " + Math.round(((EntityJupiterLander) OverlayJupiterLander.minecraft.player.getRidingEntity()).motionY * 1000) / 100.0D + " " + TranslateUtilities.translate("gui.lander.velocityu");
