...
public class GuiElectricRocketInventory extends GuiContainerGC {
	...
	public GuiElectricRocketInventory(IInventory par1IInventory, IInventory par2IInventory, EnumRocketType rocketType) {
		super(new ContainerElectricRocketInventory(par1IInventory, par2IInventory, rocketType, MCUtilities.getClient().player));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		this.fontRenderer.drawString(TranslateUtilities.translate("gui.message.power.rocket.name"), 8, 2 + 3, 4210752);
		this.fontRenderer.drawString(TranslateUtilities.translate(this.upperChestInventory.getName()), 8, 34 + 2 + 3, 4210752);
		...
		if (this.mc.player != null && this.mc.player.getRidingEntity() != null && this.mc.player.getRidingEntity() instanceof EntityElectricRocketBase) {
			this.fontRenderer.drawString(TranslateUtilities.translate("gui.message.power.message.name") + ":", 130, 15 + 3, 4210752);
			...
			this.fontRenderer.drawString(color + str, 130 - str.length() / 2, 20 + 8, 4210752);
		}
	}
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		if (this.mc.player != null && this.mc.player.getRidingEntity() != null && this.mc.player.getRidingEntity() instanceof EntityElectricRocketBase) {
			final int percentage = (int) ((int) ((((EntityElectricRocketBase) this.mc.player.getRidingEntity()).getCurrentPowerCapacity() / 100) / 2) / 2.6);
