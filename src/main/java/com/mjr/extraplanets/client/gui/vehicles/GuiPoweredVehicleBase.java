...
public class GuiPoweredVehicleBase extends GuiContainerGC {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		this.fontRenderer.drawString(TranslateUtilities.translate("gui.message.power.name"), 8, 2 + 3, 4210752);
		this.fontRenderer.drawString(TranslateUtilities.translate(this.upperChestInventory.getName()), 8, this.type == 0 ? 50 : 39, 4210752);
		...
		if (this.mc.player != null && this.mc.player.getRidingEntity() != null && this.mc.player.getRidingEntity() instanceof EntityPoweredVehicleBase) {
			...
			this.fontRenderer.drawString(color + str, 140 - str.length() / 2, 20 + 8, 4210752);
		}
	}
