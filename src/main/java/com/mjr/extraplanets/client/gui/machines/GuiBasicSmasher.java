...
public class GuiBasicSmasher extends GuiContainerGC {
	...
	public GuiBasicSmasher(InventoryPlayer par1InventoryPlayer, TileEntityBasicSmasher tileEntity) {
		super(new ContainerBasicSmasher(par1InventoryPlayer, tileEntity, FMLClientHandler.instance().getClient().player));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (!this.tileEntity.hasInputs()) {
			...
			if (this.tileEntity.canProcess() && this.tileEntity.canOutput())
				progress = 100 - this.tileEntity.processTicks * 2;
			else
				progress = 0;
			displayText = EnumColor.BRIGHT_GREEN + GCCoreUtil.translate("gui.status.smasher.name") + " " + progress + "%";
		} else {
			displayText = EnumColor.AQUA + GCCoreUtil.translate("gui.status.idle.name");
		}

		this.fontRendererObj.drawString(GCCoreUtil.translate("gui.message.status.name") + ": " + displayText, 65 - (displayText.length() * 2), 45 + 23 + yOffset, 4210752);
