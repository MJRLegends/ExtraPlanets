...
public class GUIModuleManager extends GuiContainerGC {
	...
	public void checkInputInstalled(int startX, int startY, int mouseX, int mouseY) {
		for (int i = 0; i < 4; i++) {
			startY = this.guiTop + 29 + (i * this.iconSize) + (i * this.ySpacingInstalled);
			for (int j = 0; j < ModuleHelper.getModules(MCUtilities.getClient().thePlayer.inventory.armorItemInSlot(i)).size(); j++) {
				if (checkInputForIcon(j, startX, startY, mouseX, mouseY)) {
					this.selectedModule = ModuleHelper.getModules(MCUtilities.getClient().thePlayer.inventory.armorItemInSlot(i)).get(j);
