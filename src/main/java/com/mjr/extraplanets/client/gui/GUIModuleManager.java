...
public class GUIModuleManager extends GuiContainerGC {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		this.fontRendererObj.drawString(TranslateUtilities.translate("gui.module.description.name"), 167, 40, this.mainColour);
		this.fontRendererObj.drawString(TranslateUtilities.translate("gui.module.name.name"), 167, 5, this.mainColour);
		...
		this.fontRendererObj.drawString(TranslateUtilities.translate("gui.module.power.name"), 167, 140, this.mainColour);
	}
	...
	public void renderModules(int slot, int x, int y) {
		int xOffsetLocal = 0;
		ItemStack module = MCUtilities.getClient().thePlayer.inventory.armorItemInSlot(slot);
	...
	public void renderCostList(int x, int y) {
		...
		for (ItemStack stack : this.selectedInstallModule.getRequirements()) {
			...
			this.fontRendererObj.drawString("" + stack.stackSize + " x ", x, y + yOffsetLocal, this.mainColour);
			yOffsetLocal += 18;
		}
	}
	...
	public void checkInputForIcon(int slot, int startX, int startY, int mouseX, int mouseY) {
		for (int i = 0; i < ModuleHelper.getModules(MCUtilities.getClient().thePlayer.inventory.armorItemInSlot(slot)).size(); i++) {
			if (i != 0)
				startX = startX + 8;
			if ((mouseX >= startX + (i * 16) && mouseX <= startX + 16 + (i * 16)) && (mouseY >= startY && mouseY <= startY + 16)) {
				this.selectedModule = ModuleHelper.getModules(MCUtilities.getClient().thePlayer.inventory.armorItemInSlot(slot)).get(i);
