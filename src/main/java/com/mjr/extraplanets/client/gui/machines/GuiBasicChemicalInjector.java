...
public class GuiBasicChemicalInjector extends GuiContainerGC {
	...
	public GuiBasicChemicalInjector(InventoryPlayer par1InventoryPlayer, TileEntityBasicChemicalInjector tileEntity) {
		super(new ContainerBasicChemicalInjector(par1InventoryPlayer, tileEntity, FMLClientHandler.instance().getClient().player));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (!this.tileEntity.hasInputs()) {
			...
			if (this.tileEntity.canProcess() && this.tileEntity.canOutput())
				...
				progress = 0;
			displayText = EnumColor.BRIGHT_GREEN + GCCoreUtil.translate("gui.status.injecting.name") + " " + progress + "%";
		} else {
