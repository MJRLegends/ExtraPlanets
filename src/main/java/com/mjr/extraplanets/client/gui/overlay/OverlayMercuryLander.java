...
public class OverlayMercuryLander extends Overlay {
	...
	public static void renderLanderOverlay() {
		...
		if (OverlayMercuryLander.minecraft.thePlayer.getRidingEntity().motionY < -2.0) {
			OverlayMercuryLander.minecraft.fontRendererObj.drawString(TranslateUtilities.translate("gui.warning"), width / 4 - OverlayMercuryLander.minecraft.fontRendererObj.getStringWidth(TranslateUtilities.translate("gui.warning")) / 2, height / 8 - 20,
		...
		if (OverlayMercuryLander.minecraft.thePlayer.getRidingEntity().motionY != 0.0D) {
			String string = TranslateUtilities.translate("gui.lander.velocity") + ": " + Math.round(((EntityMercuryLander) OverlayMercuryLander.minecraft.thePlayer.getRidingEntity()).motionY * 1000) / 100.0D + " "
					+ TranslateUtilities.translate("gui.lander.velocityu");
			int color = ColorUtil.to32BitColor(255, (int) Math.floor(Math.abs(OverlayMercuryLander.minecraft.thePlayer.getRidingEntity().motionY) * 51.0D),
					255 - (int) Math.floor(Math.abs(OverlayMercuryLander.minecraft.thePlayer.getRidingEntity().motionY) * 51.0D), 0);
