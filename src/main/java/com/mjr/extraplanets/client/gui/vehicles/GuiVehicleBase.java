...
public class GuiVehicleBase extends GuiContainerGC {
	...
	static {
		for (int i = 0; i < 4; i++) {
			GuiVehicleBase.sealerTexture[i] = new ResourceLocation(Constants.ASSET_PREFIX, "textures/gui/buggy_" + i * 18 + ".png");
		}
	}
	...
	public GuiVehicleBase(IInventory par1IInventory, IInventory par2IInventory, int type) {
		super(new ContainerVehicleBase(par1IInventory, par2IInventory, type, FMLClientHandler.instance().getClient().thePlayer));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (this.mc.thePlayer != null && this.mc.thePlayer.getRidingEntity() != null && this.mc.thePlayer.getRidingEntity() instanceof EntityVehicleBase) {
			this.fontRendererObj.drawString(GCCoreUtil.translate("gui.message.fuel.name") + ":", 125, 15 + 3, 4210752);
			final double percentage = ((EntityVehicleBase) this.mc.thePlayer.getRidingEntity()).getScaledFuelLevel(100);
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		this.mc.getTextureManager().bindTexture(GuiVehicleBase.sealerTexture[this.type]);
		...
		if (this.mc.thePlayer != null && this.mc.thePlayer.getRidingEntity() != null && this.mc.thePlayer.getRidingEntity() instanceof EntityVehicleBase) {
			final int fuelLevel = ((EntityVehicleBase) this.mc.thePlayer.getRidingEntity()).getScaledFuelLevel(38);
