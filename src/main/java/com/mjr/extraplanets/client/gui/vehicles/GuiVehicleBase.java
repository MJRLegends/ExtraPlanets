...
public class GuiVehicleBase extends GuiContainerGC {
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (this.mc.thePlayer != null && this.mc.thePlayer.ridingEntity != null && this.mc.thePlayer.ridingEntity instanceof EntityVehicleBase) {
			this.fontRendererObj.drawString(GCCoreUtil.translate("gui.message.fuel.name") + ":", 125, 15 + 3, 4210752);
			final double percentage = ((EntityVehicleBase) this.mc.thePlayer.ridingEntity).getScaledFuelLevel(100);
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		if (this.mc.thePlayer != null && this.mc.thePlayer.ridingEntity != null && this.mc.thePlayer.ridingEntity instanceof EntityVehicleBase) {
			final int fuelLevel = ((EntityVehicleBase) this.mc.thePlayer.ridingEntity).getScaledFuelLevel(38);
