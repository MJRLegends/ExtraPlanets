...
public class GuiVehicleBase extends GuiContainerGC {
	...
	public GuiVehicleBase(IInventory par1IInventory, IInventory par2IInventory, int type) {
		super(new ContainerVehicleBase(par1IInventory, par2IInventory, type, FMLClientHandler.instance().getClient().player));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (this.mc.player != null && this.mc.player.getRidingEntity() != null && this.mc.player.getRidingEntity() instanceof EntityVehicleBase) {
			...
			final double percentage = ((EntityVehicleBase) this.mc.player.getRidingEntity()).getScaledFuelLevel(100);
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		if (this.mc.player != null && this.mc.player.getRidingEntity() != null && this.mc.player.getRidingEntity() instanceof EntityVehicleBase) {
			final int fuelLevel = ((EntityVehicleBase) this.mc.player.getRidingEntity()).getScaledFuelLevel(38);
