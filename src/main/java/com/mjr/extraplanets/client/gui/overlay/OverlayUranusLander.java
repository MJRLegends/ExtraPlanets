...
public class OverlayUranusLander extends Overlay {
	...
	public static void renderLanderOverlay() {
		...
		if (OverlayUranusLander.minecraft.thePlayer.getRidingEntity().motionY != 0.0D) {
			String string = TranslateUtilities.translate("gui.lander.velocity") + ": " + Math.round(((EntityUranusLander) OverlayUranusLander.minecraft.thePlayer.getRidingEntity()).motionY * 1000) / 100.0D + " "
					+ TranslateUtilities.translate("gui.lander.velocityu");
			int color = ColorUtil.to32BitColor(255, (int) Math.floor(Math.abs(OverlayUranusLander.minecraft.thePlayer.getRidingEntity().motionY) * 51.0D),
					255 - (int) Math.floor(Math.abs(OverlayUranusLander.minecraft.thePlayer.getRidingEntity().motionY) * 51.0D), 0);
