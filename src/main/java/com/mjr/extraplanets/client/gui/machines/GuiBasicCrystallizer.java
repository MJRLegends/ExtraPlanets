...
public class GuiBasicCrystallizer extends GuiContainerGC {
	...
	public GuiBasicCrystallizer(InventoryPlayer par1InventoryPlayer, TileEntityBasicCrystallizer tileEntity) {
		super(new ContainerBasicCrystallizer(par1InventoryPlayer, tileEntity, MCUtilities.getClient().thePlayer));
	...
	public void initGui() {
		...
		inputTankDesc.add(TranslateUtilities.translate("gui.fuel_tank.desc.4"));
		...
		inputTankDesc.add(EnumColor.YELLOW + TranslateUtilities.translate("gui.message.fuel.name") + ": " + fuelLevel + " / " + fuelCapacity);
		...
		fuelSlotDesc.add(TranslateUtilities.translate("gui.salt_water_input.desc.0"));
		fuelSlotDesc.add(TranslateUtilities.translate("gui.salt_water_input.desc.1"));
		fuelSlotDesc.add(TranslateUtilities.translate("gui.salt_water_input.desc.2"));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		if (!this.tileEntity.hasInputs()) {
			...
			if (this.tileEntity.canProcess() && this.tileEntity.canCrystallize())
				...
				progress = 0;
			displayText = EnumColor.BRIGHT_GREEN + TranslateUtilities.translate("gui.status.crystallizing.name") + " " + progress + "%";
		} else {
			displayText = EnumColor.AQUA + TranslateUtilities.translate("gui.status.idle.name");
		}

		this.fontRendererObj.drawString(TranslateUtilities.translate("gui.message.status.name") + ": " + displayText, 70 - (displayText.length() * 2), 45 + 23 + yOffset, 4210752);
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
		...
		List<String> inputTankDesc = new ArrayList<String>();
		inputTankDesc.add(TranslateUtilities.translate("gui.salt_water_tank.desc.4"));
		int fuelLevel = this.tileEntity.inputTank != null && this.tileEntity.inputTank.getFluid() != null ? this.tileEntity.inputTank.getFluid().amount : 0;
		int fuelCapacity = this.tileEntity.inputTank != null ? this.tileEntity.inputTank.getCapacity() : 0;
		inputTankDesc.add(EnumColor.YELLOW + TranslateUtilities.translate("gui.message.salt_water.name") + ": " + fuelLevel + " / " + fuelCapacity);
		this.inputTankRegion.tooltipStrings = inputTankDesc;
