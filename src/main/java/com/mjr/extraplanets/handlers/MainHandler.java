	...
	public void onPlayer(PlayerTickEvent event) {
		if (event.player.worldObj.provider.dimensionId == Config.jupiterID) {
			...
			if (lightingSpawnChance == 10)
				event.player.worldObj.addWeatherEffect(new EntityLightningBolt(event.player.worldObj, event.player.posX + addX,event.player.worldObj.getHeightValue((int)event.player.posX + addX, (int)event.player.posZ + addZ), event.player.posZ + addZ));
		}
	}
