...
public class AchievementEventHandler {
	...
	public void onEntityDealth(LivingDeathEvent event) {
		if (event.getSource().getSourceOfDamage() instanceof EntityPlayer) {
			...
			} else if (event.getEntity() instanceof EntityEvolvedFireBatBoss) {
			...
			} else if (event.getEntity() instanceof EntityEvolvedSpacemanBoss) {
				player.addStat(ExtraPlanets_Achievements.pluto_boss, 1);
			} else if (event.getEntity() instanceof EntityEvolvedGiantZombieBoss) {
	...
	public void onCraftingRockets(EntityItemPickupEvent event) {
		if (event.getItem().getEntityItem().getItem() == AsteroidsItems.tier3Rocket)
			event.getEntityPlayer().addStat(ExtraPlanets_Achievements.craft_rocket_3, 1);
		else if (event.getItem().getEntityItem().getItem() == ExtraPlanets_Items.TIER_4_ROCKET)
