	...
	public void onSleepInBedEvent(PlayerWakeUpEvent event) {
		...
		if (player.worldObj.isRemote == false && (!event.wakeImmediately() && !event.updateWorld())) {
			...
			if (level <= 0)
				...
				player.addChatMessage(new TextComponentString("" + TextFormatting.AQUA + TextFormatting.BOLD + playerMP.getName() + TextFormatting.GOLD + ", " + GCCoreUtil.translate("gui.radiation.reduced.message") + " 5%"));
				player.addChatMessage(new TextComponentString("" + TextFormatting.AQUA + TextFormatting.BOLD + playerMP.getName() + TextFormatting.DARK_AQUA + ", " + GCCoreUtil.translate("gui.radiation.current.message") + ": "
	...
	public void onWorldChange(PlayerChangedDimensionEvent event) {
		if (event.player.worldObj.isRemote == false) {
			if (event.player.worldObj.provider instanceof WorldProviderRealisticSpace) {
				...
				player.addChatMessage(new TextComponentString("" + TextFormatting.AQUA + TextFormatting.BOLD + player.getName() + TextFormatting.GOLD + ", " + GCCoreUtil.translate("gui.radiation.cancel.message") + "!"));
			}
		}
	}
}
