	...
	public void onSleepInBedEvent(PlayerWakeUpEvent event) {
		...
		if (player.worldObj.isRemote == false && (!event.wakeImmediately() && !event.updateWorld())) {
			...
			if (level <= 0)
				...
				player.addChatMessage(new TextComponentString("" + TextFormatting.AQUA + TextFormatting.BOLD + playerMP.getName() + TextFormatting.DARK_AQUA + ", " + GCCoreUtil.translate("gui.radiation.current.message") + ": " + (int) stats.getRadiationLevel() + "%"));
			}
		}
	}
