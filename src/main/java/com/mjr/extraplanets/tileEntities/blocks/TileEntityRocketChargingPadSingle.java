		...
		if (!this.worldObj.isRemote && this.corner == 0) {
			...
			if (attachedLaunchPads.size() == 25) {
				for (final TileEntity tile : attachedLaunchPads) {
					...
					((TileEntityRocketChargingPadSingle)tile).corner = 1;
				}

				this.getPos().south(1);
				this.getPos().east(1);

				this.worldObj.setBlockState(this.getPos(), ExtraPlanets_Blocks.ADVANCED_LAUCHPAD_FULL.getDefaultState().withProperty(BlockCustomLandingPadFull.PAD_TYPE, BlockCustomLandingPadFull.EnumLandingPadFullType.ROCKET_POWER_CHARGING_PAD));
			}
		}
	}
