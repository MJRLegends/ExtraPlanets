		...
		if (!this.worldObj.isRemote && this.corner == 0) {
			...
			for (int x = this.getPos().getX() - 2; x < this.getPos().getX() + 3; x++) {
				for (int z = this.getPos().getZ() - 2; z < this.getPos().getZ() + 3; z++) {
					...
					if (tile instanceof TileEntityTier3LandingPadSingle && !tile.isInvalid() && ((TileEntityTier3LandingPadSingle) tile).corner == 0) {
			...
			if (attachedLaunchPads.size() == 25) {
				for (final TileEntity tile : attachedLaunchPads) {
					...
					((TileEntityTier3LandingPadSingle)tile).corner = 1;
				}

				this.getPos().south(1);
				this.getPos().east(1);

				this.worldObj.setBlockState(this.getPos(), ExtraPlanets_Blocks.ADVANCED_LAUCHPAD_FULL.getDefaultState().withProperty(BlockCustomLandingPadFull.PAD_TYPE, BlockCustomLandingPadFull.EnumLandingPadFullType.TIER_3_ROCKET_PAD));
			}
		}
	}
