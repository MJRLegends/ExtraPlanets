...
public class ElectricItemManagerIC2 implements IElectricItemManager {
	...
	public double charge(ItemStack itemStack, double amount, int tier, boolean ignoreTransferLimit, boolean simulate) {
		if (itemStack.getItem() instanceof ElectricArmorBase) {
			ElectricArmorBase item = (ElectricArmorBase) itemStack.getItem();
	...
	public double getMaxCharge(ItemStack stack) {
		if (stack.getItem() instanceof ElectricArmorBase) {
			return ((ElectricArmorBase) stack.getItem()).getMaxCharge(stack);
		}
		return 1;
	}
	...
	public int getTier(ItemStack stack) {
		if (stack.getItem() instanceof ElectricArmorBase) {
			return ((ElectricArmorBase) stack.getItem()).getTier(stack);
		}
