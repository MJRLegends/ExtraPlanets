...
public class Tier4SpaceSuitArmor extends ItemArmor implements IPressureSuit, IRadiationSuit, IArmorGravity, IBreathableArmor, IModularArmor  {
	public String name;
	...
	public int gravityOverrideIfLow(EntityPlayer p) {
		for (int i = 0; i < 4; i++)
			if (p.inventory.armorInventory.get(i) != null)
				if (p.inventory.armorInventory.get(i).getItem() == ExtraPlanets_Armor.TIER_4_SPACE_SUIT_GRAVITY_BOOTS)
					return 55;
		return 0;
	}
