...
public class Tier5Rocket extends Item implements IHoldableItem {
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (world.isRemote && player instanceof EntityPlayerSP) {
			...
			if (padFound) {
				...
				final EntityTier5Rocket spaceship = new EntityTier5Rocket(world, centerX, centerY, centerZ, EnumRocketType.values()[itemStack.getItemDamage()]);
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List<String> par2List, boolean b) {
		...
		if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey("RocketFuel")) {
			EntityTier5Rocket rocket = new EntityTier5Rocket(FMLClientHandler.instance().getWorldClient(), 0, 0, 0, EnumRocketType.values()[itemStack.getItemDamage()]);
	...
	public String getUnlocalizedName(ItemStack itemStack) {
		return super.getUnlocalizedName(itemStack) + ".t5Rocket";
	}
