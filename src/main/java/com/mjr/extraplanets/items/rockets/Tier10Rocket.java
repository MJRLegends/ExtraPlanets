		...
		if (par3World.isRemote) {
			...
			for (int i = -1; i < 4; i++) {
				for (int j = -1; j < 4; j++) {
					...
					if (id == ExtraPlanets_Blocks.advancedLaunchPadFull && meta == 1) {
						padFound = true;
			...
			if (padFound) {
				...
				EntityTier10Rocket rocket = new EntityTier10Rocket(par3World, centerX, centerY, centerZ, EnumRocketType.values()[par1ItemStack.getItemDamage()]);
