...
public class Tier9Rocket extends Item implements IHoldableItem {
	public Tier9Rocket(String name) {
		super();
		this.setMaxDamage(0);
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (world.isRemote && player instanceof EntityPlayerSP) {
			...
			if (padFound) {
				...
				final EntityTier9Rocket spaceship = new EntityTier9Rocket(world, centerX, centerY, centerZ, EnumRocketType.values()[itemStack.getItemDamage()]);
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List<String> par2List, boolean b) {
		...
		if (itemStack.hasTagCompound() && itemStack.getTagCompound().hasKey("RocketFuel")) {
			EntityTier9Rocket rocket = new EntityTier9Rocket(FMLClientHandler.instance().getWorldClient(), 0, 0, 0, EnumRocketType.values()[itemStack.getItemDamage()]);
	...
	public String getUnlocalizedName(ItemStack itemStack) {
		return super.getUnlocalizedName(itemStack) + ".t9Rocket";
	}
