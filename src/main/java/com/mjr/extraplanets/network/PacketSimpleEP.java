		...
		case C_OPEN_PARACHEST_GUI:
			...
			case 0:
				if (player.ridingEntity instanceof EntityVehicleBase) {
					MCUtilities.getClient().displayGuiScreen(new GuiVehicleBase(player.inventory, (EntityVehicleBase) player.ridingEntity, ((EntityVehicleBase) player.ridingEntity).getType()));
					player.openContainer.windowId = (Integer) this.data.get(0);
				} else if (player.ridingEntity instanceof EntityPoweredVehicleBase) {
					MCUtilities.getClient().displayGuiScreen(new GuiPoweredVehicleBase(player.inventory, (EntityPoweredVehicleBase) player.ridingEntity, ((EntityPoweredVehicleBase) player.ridingEntity).getType()));
					player.openContainer.windowId = (Integer) this.data.get(0);
				}
				break;
			}
