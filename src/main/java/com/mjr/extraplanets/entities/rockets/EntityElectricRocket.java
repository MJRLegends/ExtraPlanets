...
public class EntityElectricRocket extends EntityElectricRocketBase {
	public EntityElectricRocket(World par1World) {
		...
		this.setPowerMaxCapacity(20000);
		this.setCurrentPowerCapacity(0);
	}

	public EntityElectricRocket(World world, double x, double y, double z, EnumRocketType rocketType) {
		super(world, x, y, z);
	...
	public void onUpdate() {
		...
		if ((this.getLaunched() || this.launchPhase == EnumLaunchPhase.IGNITED.ordinal() && this.rand.nextInt(i) == 0) && !ConfigManagerCore.disableSpaceshipParticles && this.hasValidPower()) {
			if (this.world.isRemote) {
				this.spawnParticles(this.getLaunched());
			}
		}

		if (this.launchPhase >= EnumLaunchPhase.LAUNCHED.ordinal() && this.hasValidPower()) {
		...
		} else if (!this.hasValidPower() && this.getLaunched() && !this.world.isRemote) {
	...
	public boolean isDockValid(IPowerDock dock) {
		return dock instanceof TileEntityRocketChargingPad;
	}
}
