...
public class EntityTier10Rocket extends EntityTieredRocket {
	public EntityTier10Rocket(World par1World) {
		super(par1World);
		this.setSize(5.2F, 13.0F);
		this.yOffset = 1.5F;
	}

	public EntityTier10Rocket(World par1World, double par2, double par4, double par6, EnumRocketType rocketType) {
	...
	protected void onRocketLand(int x, int y, int z) {
		this.setPositionAndRotation(x + 0.5, y + 2.0D, z + 1.2, this.rotationYaw, 0.0F);
	...
	public ItemStack getPickedResult(MovingObjectPosition target) {
		return new ItemStack(ExtraPlanets_Items.tier10Rocket, 1, this.rocketType.getIndex());
	}
	...
	public void onTeleport(EntityPlayerMP player) {
		...
		if (playerBase != null) {
			...
			stats.rocketItem = ExtraPlanets_Items.tier10Rocket;
    ...
    public void onLaunch()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (stats != null)
            {
                stats.launchpadStack = amountRemoved == 9 ? new ItemStack(ExtraPlanets_Blocks.advancedLaunchPadFull, 25, 1) : null;
            }
