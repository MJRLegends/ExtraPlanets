...
public abstract class EntityTieredRocketCustom extends EntityAutoRocket implements IRocketType, IDockable, IInventory, IWorldTransferCallback, ICameraZoomEntity {
	...
	public EntityTieredRocketCustom(World par1World) {
		super(par1World);
		this.setSize(0.98F, 4F);
		this.yOffset = this.height / 2.0F;
	}

	public EntityTieredRocketCustom(World world, double posX, double posY, double posZ) {
	...
	protected void entityInit() {
		...
		// TODO reimplement once Resonant Engine comes out of alpha, bug
		// DarkGuardsman for info
	...
	public void setDead() {
		...
		// TODO reimplement once Resonant Engine comes out of alpha, bug Dark
		// for info
		// if (Loader.isModLoaded("ICBM|Explosion"))
		// {
		// try
	...
	public void onUpdate() {
		...
		if (this.riddenByEntity != null) {
			int tier = 0;
			if (this.getRocketTier() == 7 || this.getRocketTier() == 8 || this.getRocketTier() == 9)
				tier = 5;
			else
				tier = this.getRocketTier();
			this.riddenByEntity.posX += this.rumble / (37F - 5F * tier);
			this.riddenByEntity.posZ += this.rumble / (37F - 5F * tier);
		}
					...
					if (targetDim != null && targetDim.worldObj instanceof WorldServer) {
						...
						if (dimensionAllowed) {
							if (this.riddenByEntity != null) {
								...
								// Now destroy the rocket entity, the rider is
