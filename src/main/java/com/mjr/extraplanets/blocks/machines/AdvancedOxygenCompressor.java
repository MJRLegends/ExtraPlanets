...
public class AdvancedOxygenCompressor extends BlockAdvancedTile implements ItemBlockDesc.IBlockShiftDesc {
	...
	public void registerBlockIcons(IIconRegister par1IconRegister) {
		...
		this.iconCompressor1 = par1IconRegister.registerIcon(Constants.TEXTURE_PREFIX + "advanced_oxygen_compressor");
		this.iconCompressor2 = par1IconRegister.registerIcon(GalacticraftCore.TEXTURE_PREFIX + "machine_compressor_2");
		this.iconDecompressor = par1IconRegister.registerIcon(Constants.TEXTURE_PREFIX + "advanced_oxygen_decompressor");
	...
	public boolean onUseWrench(World par1World, int x, int y, int z, EntityPlayer par5EntityPlayer, int side, float hitX, float hitY, float hitZ) {
		...
		if (metadata >= AdvancedOxygenCompressor.OXYGEN_DECOMPRESSOR_METADATA) {
			original -= AdvancedOxygenCompressor.OXYGEN_DECOMPRESSOR_METADATA;
		} else if (metadata >= AdvancedOxygenCompressor.OXYGEN_COMPRESSOR_METADATA) {
			original -= AdvancedOxygenCompressor.OXYGEN_COMPRESSOR_METADATA;
		}
		...
		if (metadata >= AdvancedOxygenCompressor.OXYGEN_DECOMPRESSOR_METADATA) {
			meta += AdvancedOxygenCompressor.OXYGEN_DECOMPRESSOR_METADATA;
		} else if (metadata >= AdvancedOxygenCompressor.OXYGEN_COMPRESSOR_METADATA) {
			meta += AdvancedOxygenCompressor.OXYGEN_COMPRESSOR_METADATA;
		}
	...
	public IIcon getIcon(int side, int metadata) {
		...
		if (metadata >= AdvancedOxygenCompressor.OXYGEN_DECOMPRESSOR_METADATA) {
			metadata -= AdvancedOxygenCompressor.OXYGEN_DECOMPRESSOR_METADATA;
			...
			metadata -= AdvancedOxygenCompressor.OXYGEN_COMPRESSOR_METADATA;
	...
	public String getShiftDescription(int meta) {
		...
		case OXYGEN_COMPRESSOR_METADATA:
			return GCCoreUtil.translate("tile.advanced.oxygenCompressor.description");
		case OXYGEN_DECOMPRESSOR_METADATA:
			return GCCoreUtil.translate("tile.advanced.oxygenDecompressor.description");
		}
		return "";
	}
