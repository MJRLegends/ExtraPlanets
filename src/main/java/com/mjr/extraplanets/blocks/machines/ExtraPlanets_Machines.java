...
public class ExtraPlanets_Machines {
	...
	private static void registerMachines() throws NoSuchMethodException {
		if (Config.REFINERY_ADVANCED)
			RegisterUtilities.registerBlock(Constants.modID, REFINERY_ADVANCED, REFINERY_ADVANCED.getUnlocalizedName().substring(5));
		if (Config.REFINERY_ULTIMATE)
			RegisterUtilities.registerBlock(Constants.modID, REFINERY_ULTIMATE, REFINERY_ULTIMATE.getUnlocalizedName().substring(5));
		if (Config.SOLAR_PANELS)
