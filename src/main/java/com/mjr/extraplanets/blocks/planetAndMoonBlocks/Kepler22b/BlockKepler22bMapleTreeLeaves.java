...
public class BlockKepler22bMapleTreeLeaves extends BlockLeavesBase implements IShearable {
	...
	public void registerBlockIcons(IIconRegister iconReg) {
		...
		for (int i = 0; i < leaves.length; ++i) {
			this.leafTextures[0][i] = iconReg.registerIcon(Constants.TEXTURE_PREFIX + name.replace("_new", "") + "_leaves_" + leaves[i]);
			this.leafTextures[1][i] = iconReg.registerIcon(Constants.TEXTURE_PREFIX + name.replace("_new", "") + "_leaves_" + leaves[i]);
		}
	}
