...
IInventory, ISidedInventory, IConnector {
	...
	if (!this.worldObj.isRemote) {
		...
		if (this.worldObj.isDaytime()
			&& (this.worldObj.provider instanceof IGalacticraftWorldProvider || !this.worldObj.isRaining()
	...
	if (worldObj.isDaytime())
	    ...
	    return MathHelper.floor_float(0.01F * difference * (this.solarStrength * (Math.abs(difference) * 500.0F))
		    * this.getSolarBoost());
    }
	...
	for (int y = 1; y <= 2; y++) {
		...
		for (int z = -1; z < 2; z++) {
			...
			FMLClientHandler.instance().getClient().effectRenderer.addBlockDestroyEffects(thisBlock.x + (y == 2 ? x : 0),
				thisBlock.y + y, thisBlock.z + (y == 2 ? z : 0), ExtraPlanetsBlocks.solarPanel,
				Block.getIdFromBlock(ExtraPlanetsBlocks.solarPanel) >> 12 & 255);
		    }
