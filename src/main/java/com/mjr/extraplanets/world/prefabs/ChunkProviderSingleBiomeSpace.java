	...
	public void generateTerrain(int chunkX, int chunkZ, ChunkPrimer primer) {
		...
		for (int x = 0; x < ChunkProviderSingleBiomeSpace.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderSingleBiomeSpace.CHUNK_SIZE_Z; z++) {
				...
				final double featureFilter = this.noiseGen5.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderSingleBiomeSpace.MAIN_FEATURE_FILTER_MOD;
				final double largeFilter = this.noiseGen6.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderSingleBiomeSpace.LARGE_FEATURE_FILTER_MOD;
				final double smallFilter = this.noiseGen7.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderSingleBiomeSpace.SMALL_FEATURE_FILTER_MOD - 0.5;
				...
				for (int y = 0; y < ChunkProviderSingleBiomeSpace.CHUNK_SIZE_Y; y++) {
	...
	public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn) {
		...
		for (int var8 = 0; var8 < 16; ++var8) {
			for (int var9 = 0; var9 < 16; ++var9) {
				...
				for (int var16 = ChunkProviderSingleBiomeSpace.CHUNK_SIZE_Y - 1; var16 >= 0; --var16) {
